...
if (nb_email_field) {
  ...
  function setEmailStatus(status) {
    var existing_errors = nb_email_field_wrapper.parentNode.parentNode.getElementsByClassName(
      "en__field__error"
    );
    if (existing_errors.length) {
      // remove the existing default error and replace with one that fits in this custom framework
      nb_email_field_wrapper.parentNode.parentNode.removeChild(
        existing_errors.item(0)
      );
...
setTimeout(function() {
  if (typeof _nb == "undefined") {
    window.en_nb_valid_email = true;
    // Send POST to Notification Script
    var form_data = {
    ...
    xhr.open(
      "PUT",
      "https://apps.4sitestudios.com/fernando/oc/neverbounce-notification/notify.php",
      true
    );
